# limit-calculation Specification

## Purpose
Kontrolgrænseberegning per diagramtype: centerlinje, sigmaestimat og
punktvise grænser, som `viewModelClass.calculateLimits()` kalder én gang per
fase med fasens baseline (`subset_points`). Første requirements dækker I′
(`ip`); øvrige typers regler tilføjes ved behov.
Oprettet ved arkivering af `add-i-prime-chart` (2026-09-16).
