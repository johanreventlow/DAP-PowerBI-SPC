...
// includes the Anhoej Rules group with all six descriptors. If this passes
// but the group is still missing in Power BI Desktop, the bug is host-side
...
describe("FormattingModel — Anhoej Rules group", () => {
    ...
    it("Anhoej Rules group contains both toggles plus four color descriptors", () => {
        const anhoejGroup = outliersCard?.groups.find(g => g.displayName === "Anhoej Rules");
        ...
        expect(sliceProps).toContain("anhoj_long_run_colour_improvement");
        expect(sliceProps).toContain("anhoj_long_run_colour_deterioration");
        expect(sliceProps).toContain("anhoj_long_run_colour_neutral_low");
        expect(sliceProps).toContain("anhoj_long_run_colour_neutral_high");
    });
